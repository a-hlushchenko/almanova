...
add_action('wpcf7_mail_sent', function ($cf7) {
	...
	if(isset($data['firstname']) && !empty($data['firstname'])){
		$txt .= "First name: " . strip_tags(trim(urlencode($data['firstname']))) . "%0A";
	}
	if(isset($data['lastname']) && !empty($data['lastname'])){
		$txt .= "Last name: " . strip_tags(trim(urlencode($data['lastname']))) . "%0A";
	}
	if(isset($data['username']) && !empty($data['username'])){
		$txt .= "Name: " . strip_tags(trim(urlencode($data['username']))) . "%0A";
	}
	if (isset($data['phone']) && !empty($data['phone'])) {
		$txt .= "Phone: " . strip_tags(trim(urlencode($data['phone']))) . "%0A";
	}
	if (isset($data['email']) && !empty($data['email'])) {
		$txt .= "Email: " . strip_tags(trim(urlencode($data['email']))) . "%0A";
	}
	if (isset($data['message']) && !empty($data['message'])) {
		$txt .= "Message: " . strip_tags(trim(urlencode($data['message']))) . "%0A";
	}
	if (isset($data['date']) && !empty($data['date'])) {
		$txt .= "Date: " . strip_tags(trim(urlencode($data['date']))) . "%0A";
	}
	if (isset($data['time']) && !empty($data['time'])) {
		$txt .= "Time: " . strip_tags(trim(urlencode($data['time']))) . "%0A";
	}
